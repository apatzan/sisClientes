...
class TipificacionController extends Controller
{
    ...
    public function destroy ($id){
		...
		//  Nuevo metodo para obtener de forma dinamica las tipificaciones mediante ajax 
		public function getTipificaciones(Request $request,$id)
		{
			if($request->ajax()){
			
			$tipificaciones=DB::table('tipificacion')->where('esPromesa','=',$id)->where('estado','=','1')->get();			
			return response()->json($tipificaciones);
			}
